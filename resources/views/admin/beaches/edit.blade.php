                        ...
                        <form action="{{ route('admin.beaches.update', $beach->id) }}" method="POST"
                            ...
                            <div class="mb-4">
                                ...
                                <div id="primary-drop-area"
                                    ...
                                    @endphp
                                    <img id="primaryImagePreview" src="{{ $primaryImgSrc }}"
                                        alt="Primary Preview" class="img-fluid rounded-3 mb-2 w-100"
                                    ...
                                    <div class="col-4" data-image-id="{{ $image->id }}">
                                        <div class="image-preview-item">
                                            <img src="{{ str_starts_with($image->image_url, 'http') || str_starts_with($image->image_url, '/assets') ? $image->image_url : asset('storage/' . (str_starts_with($image->image_url, 'beaches/') ? $image->image_url : 'beaches/' . $image->image_url)) }}" 
                                                 class="img-fluid rounded shadow-sm w-100" style="height:80px;object-fit:cover;">
                                            <button type="button" class="btn btn-danger btn-sm remove-btn" 
                                                    onclick="removeExistingImage({{ $image->id }})" title="Xóa ảnh này">
                                                <i class="bi bi-x"></i>
                                            </button>
                                            @if($image->is_primary)
                                                <span class="badge bg-primary position-absolute bottom-0 start-0 m-1" style="font-size:10px;">
            ...
            <div class="col-lg-6">
                <div class="card border-0 shadow-sm sticky-top" style="top: 20px;">
                    <div class="card-header bg-gradient" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white;">
                        ...
                        <small class="opacity-75">Giao diện sau khi cập nhật</small>
                    </div>
                    <div class="card-body p-4">
                        <div class="preview-container">
                            <div class="text-center mb-4">
                                ...
                                @endphp
                                <img id="previewPrimaryImage" src="{{ $primaryImgSrc }}"
                            ...
                            <div class="preview-content">
                                ...
                                <div class="description-section">
                                    <p class="text-muted mb-3 lh-lg" id="previewShortDescription">{{ $beach->short_description }}</p>
                                    <p class="mb-3" id="previewLongDescription">{{ $beach->detail->long_description ?? '' }}</p>
                                    
                                    <div class="quote-section mb-3" id="previewQuoteContainer" style="{{ $beach->detail->highlight_quote ? 'display: block;' : 'display: none;' }}">
                                        <div class="bg-light border-start border-4 border-info p-3 rounded-end">
                                            <i class="bi bi-quote text-info me-2"></i>
                                            <em id="previewHighlightQuote" class="text-info">{{ $beach->detail->highlight_quote ?? '' }}</em>
                                        </div>
                                    </div>
                                    
                                    <p class="mb-3" id="previewLongDescription2">{{ $beach->detail->long_description2 ?? '' }}</p>
