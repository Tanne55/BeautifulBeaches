            ...
            <div class="col-lg-6">
                <div class="card shadow-sm">
                    <div class="card-body p-2">
                        ...
                        @endif
                        <form action="{{ route('ceo.tours.update', $tour->id) }}" method="POST" enctype="multipart/form-data" id="tourForm">
                            @csrf
                            @method('PUT')
                            <!-- Ảnh preview -->
                            <div class="mb-4 text-center">
                                <div id="drop-area"
                                    ...
                                    <img id="imagePreview" src="{{ $tour->image ? asset($tour->image) : 'https://via.placeholder.com/900x350?text=Preview+Image' }}"
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="title" name="title" placeholder="Nhập tên tour" required value="{{ old('title', $tour->title) }}">
                            </div>
                            <!-- Giá -->
                            <div class="mb-3">
                                <label for="price" class="form-label">Giá (triệu đồng)</label>
                                <input type="number" class="form-control" id="price" name="price" placeholder="Nhập giá" required value="{{ old('price', $tour->price) }}" min="0" step="0.1">
                            </div>
                            <!-- Giá gốc -->
                            <div class="mb-3">
                                <label for="original_price" class="form-label">Giá gốc (triệu đồng)</label>
                                <input type="number" class="form-control" id="original_price" name="original_price" placeholder="Nhập giá gốc" value="{{ old('original_price', $tour->original_price) }}" min="0" step="0.1">
                            </div>
                            <!-- Sức chứa -->
                            <div class="mb-3">
                                <label for="capacity" class="form-label">Sức chứa</label>
                                <input type="number" class="form-control" id="capacity" name="capacity" placeholder="Nhập sức chứa" required value="{{ old('capacity', $tour->capacity) }}" min="1">
                            </div>
                            <!-- Thời lượng (số ngày) -->
                            <div class="mb-3">
                                <label for="duration_days" class="form-label">Thời lượng (số ngày)</label>
                                <input type="number" class="form-control" id="duration_days" name="duration_days" placeholder="Nhập số ngày" required value="{{ old('duration_days', $tour->duration_days) }}" min="1" max="30" step="1" oninput="updateReturnTime()">
                            </div>
                            <!-- Ngày giờ khởi hành -->
                            <div class="mb-3">
                            ...
                            <!-- Ngày giờ trở về (ẩn) -->
                            <input type="hidden" id="return_time" name="return_time" value="{{ old('return_time', optional($tour->detail)->return_time ? date('Y-m-d\TH:i', strtotime($tour->detail->return_time)) : '') }}">
                            <!-- Trạng thái -->
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="included_services" name="included_services" rows="2" placeholder="Nhập các dịch vụ bao gồm, mỗi dòng 1 dịch vụ">{{ old('included_services', optional($tour->detail)->included_services ? implode("\n", json_decode($tour->detail->included_services, true)) : '') }}</textarea>
                            </div>
                            <!-- Không bao gồm -->
                            <div class="mb-3">
                                <label for="excluded_services" class="form-label">Không bao gồm (mỗi dòng 1 mục)</label>
                                <textarea class="form-control" id="excluded_services" name="excluded_services" rows="2" placeholder="Nhập các mục không bao gồm, mỗi dòng 1 mục">{{ old('excluded_services', optional($tour->detail)->excluded_services ? implode("\n", json_decode($tour->detail->excluded_services, true)) : '') }}</textarea>
                            </div>
                            <!-- Điểm nổi bật -->
                            <div class="mb-3">
                                <label for="highlights" class="form-label">Điểm nổi bật (mỗi dòng 1 điểm)</label>
                                <textarea class="form-control" id="highlights" name="highlights" rows="2" placeholder="Nhập các điểm nổi bật, mỗi dòng 1 điểm">{{ old('highlights', optional($tour->detail)->highlights ? implode("\n", json_decode($tour->detail->highlights, true)) : '') }}</textarea>
                            </div>
                ...
                <div class="card shadow-sm mb-3">
                    <div class="card-body p-3">
                        <div class="text-center mb-3">
                            <img id="previewImageShow" src="{{ $tour->image ? asset($tour->image) : 'https://via.placeholder.com/900x350?text=Preview+Image' }}" alt="Preview" class="img-fluid rounded" style="max-height:350px;object-fit:cover;">
                        </div>
                        <table class="table table-bordered table-striped mb-3">
                            <tbody>
                                ...
                                <tr>
                                    <th>Giá</th>
                                    <td>
                                        <span class="text-danger fw-bold" id="previewPrice">{{ number_format($tour->price, 0, ',', '.') }}</span> triệu đồng
                                        <span class="text-decoration-line-through text-muted" id="previewOriginalPrice">@if($tour->original_price > $tour->price) ({{ number_format($tour->original_price, 0, ',', '.') }}) 
                                            @endif</span>
                                    </td>
                                </tr>
                                <tr>
                                    <th>Giờ khởi hành</th>
                                    <td id="previewDeparture">{{ optional($tour->detail)->departure_time ? date('d/m/Y H:i', strtotime($tour->detail->departure_time)) : '-' }}</td>
                                </tr>
                                <tr>
                                    <th>Giờ trở về</th>
                                    <td id="previewReturn">{{ optional($tour->detail)->return_time ? date('d/m/Y H:i', strtotime($tour->detail->return_time)) : '-' }}</td>
                                </tr>
                                <tr>
                                    <th>Dịch vụ bao gồm</th>
                                    <td><ul class="mb-0" id="previewIncludedServices">@if(optional($tour->detail)->included_services)@foreach(json_decode($tour->detail->included_services, true) as $service)<li>{{ $service }}</li>@endforeach @endif</ul></td>
                                </tr>
                                <tr>
                                    <th>Không bao gồm</th>
                                    <td><ul class="mb-0" id="previewExcludedServices">@if(optional($tour->detail)->excluded_services)@foreach(json_decode($tour->detail->excluded_services, true) as $service)<li>{{ $service }}</li>@endforeach @endif</ul></td>
                                </tr>
                                <tr>
                                    <th>Điểm nổi bật</th>
                                    <td><ul class="mb-0" id="previewHighlights">@if(optional($tour->detail)->highlights)@foreach(json_decode($tour->detail->highlights, true) as $highlight)<li>{{ $highlight }}</li>@endforeach @endif</ul></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
