    ...
    <div class="container container-custom py-4">
        <div class="row">
            <div class="col-12 col-lg-8 col-xl-6 mx-auto">
                ...
                @endif
                <form action="{{ route('ceo.tickets.update', $ticket) }}" method="POST"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="full_name" name="full_name"
                            value="{{ old('full_name', $ticket->full_name) }}" maxlength="255">
                    </div>
                    <div class="mb-3">
                        <label for="email" class="form-label">Email</label>
                        <input type="email" class="form-control" id="email" name="email"
                            value="{{ old('email', $ticket->email) }}" maxlength="255">
                    </div>
                    <div class="mb-3">
                        <label for="phone" class="form-label">Phone</label>
                        <input type="text" class="form-control" id="phone" name="phone"
                            value="{{ old('phone', $ticket->phone) }}" maxlength="20">
                    </div>
                    <div class="mb-3">
                        <label for="status" class="form-label">Status</label>
                        <select class="form-select" id="status" name="status">
                            <option value="valid" @if(old('status', $ticket->status) === 'valid') selected @endif>Valid
                            </option>
                            <option value="used" @if(old('status', $ticket->status) === 'used') selected @endif>Used</option>
                            <option value="cancelled" @if(old('status', $ticket->status) === 'cancelled') selected @endif>
                                Cancelled</option>
                        </select>
                    </div>
