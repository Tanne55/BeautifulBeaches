                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <p class="text-muted mb-0">
                                Tháng {{ Carbon::createFromFormat('Y-m', request('month', now()->format('Y-m')))->format('m/Y') }}
                            </p>
                        </div>
                    ...
                    <form method="GET" class="row g-3">
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="status" class="form-select">
                                ...
                                <option value="grouped" {{ request('status') == 'grouped' ? 'selected' : '' }}>Đã nhóm</option>
                                ...
                                <div class="row mb-4">
                                    <div class="col-md-3">
                                        <div class="text-center p-3 bg-warning bg-opacity-10 rounded">
                                            <h5 class="text-warning mb-1">{{ $pendingBookings ? $pendingBookings->count() : 0 }}</h5>
                                            <small class="text-muted">Chờ xử lý</small>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="text-center p-3 bg-info bg-opacity-10 rounded">
                                            <h5 class="text-info mb-1">{{ $groupedStatusBookings ? $groupedStatusBookings->count() : 0 }}</h5>
                                            <small class="text-muted">Đã nhóm</small>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="text-center p-3 bg-success bg-opacity-10 rounded">
                                            <h5 class="text-success mb-1">{{ $confirmedBookings ? $confirmedBookings->count() : 0 }}</h5>
                                            <small class="text-muted">Đã xác nhận</small>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="text-center p-3 bg-danger bg-opacity-10 rounded">
                                            <h5 class="text-danger mb-1">{{ $cancelledBookings ? $cancelledBookings->count() : 0 }}</h5>
                                        ...
                                        <div class="card-body">
                                            @if(!$sameDepartureDate)
                                                <div class="alert alert-warning">
                                                    ...
                                                    <p class="mb-0">Các booking có ngày khởi hành khác nhau. Chỉ có thể gom nhóm các booking có cùng ngày khởi hành.</p>
                                                </div>
                                            @else
                                                <div class="alert alert-info">
                                                    ...
                                                    <ul class="mb-0 small">
                                                        ...
                                                        <li>Chỉ có thể gom các bookings có trạng thái "Chờ xử lý" và cùng ngày khởi hành</li>
                                                    </ul>
                                                </div>
                                            @endif

                                            <form action="{{ route('ceo.bookings.createGroup') }}" method="POST" id="groupForm_{{ $loop->index }}">
                                                @csrf
                                                ...
                                                <div class="row mb-3">
                                                    ...
                                                    <div class="col-md-6 text-end">
                                                        <span class="text-muted">
                                                            Đã chọn: <span id="selectedCount_{{ $loop->index }}">0</span> / {{ $pendingBookings ? $pendingBookings->count() : 0 }}
                                                        </span>
                                                    </div>
                                                </div>
                                                                ...
                                                                <div class=" border">
                                                                    <div class="card-body p-3">
                                                                        <div class="form-check">
                                                                            <input class="form-check-input booking-checkbox-{{ $loop->parent->index }}" 
                                                                                   type="checkbox" 
                                                                                   name="booking_ids[]" 
                                                                                   value="{{ $booking->id }}" 
                                    ...
                                    <div class="col-md-6">
                                        <div class=" bg-warning bg-opacity-10 border-warning">
                                            <div class="card-body text-center">
                                                ...
                                                <h4 class="text-warning mb-0">{{ number_format($totalPendingAmount, 0, ',', '.') }}đ</h4>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
...
@if(isset($bookings) && count($bookings) > 0)
    @foreach($bookings as $booking)
        ...
        <div class="offcanvas offcanvas-end" 
             tabindex="-1" 
             id="ticketsOffcanvas{{ $booking->id }}"
             aria-labelledby="ticketsOffcanvasLabel{{ $booking->id }}" 
             style="width: 700px;">
                        ...
                        <tbody>
                            @foreach($booking->tickets as $i => $ticket)
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            <a href="{{ route('ceo.tickets.show', $ticket) }}" 
                                               class="btn btn-sm btn-outline-info" 
                                               title="Xem chi tiết">
